...
<c:catch var="catchtheException">
    <sql:update dataSource="${db}" var="i">
        DELETE FROM `gambo_products` WHERE `product_id`='${param.product_id}'
    </sql:update>
</c:catch>

<c:if test="${i == 1}">
    <c:set value="Produuct is successfully Deteled" var="toast_message" scope="session"/>

</c:if>
<c:if test="${catchtheException !=null}">
    <c:set value="Opps! some Error occured while Deleting Product" var="toast_message" scope="session"/>
