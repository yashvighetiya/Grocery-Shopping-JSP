...
<c:catch var="exception">
    ...
    UPDATE `categories` SET `category_image_path`='${imagepath}' WHERE `category_id`=${category_id}
  </sql:update>
</c:catch>
  ...
  <c:if test="${update == 1}">
      <c:remove var="category_id"/>
      <c:set var="toast_message" scope="session" value="Category Edit SuccessFully"/>
     <c:redirect url="category.jsp"/>
  </c:if>